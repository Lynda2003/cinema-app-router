...
function AjouterFilm({ onAjouter }) {
  ...
  function valider() {
    ...
    if (!form.posterURL.trim()) {
      ...
      try { new URL(form.posterURL); } catch { nouvelles.posterURL = "URL invalide."; }
    }
    if (!form.trailerURL.trim()) {
      nouvelles.trailerURL = "L'URL de la bande-annonce est requise.";
    } else {
      try { new URL(form.trailerURL); } catch { nouvelles.trailerURL = "URL invalide."; }
    }
    return nouvelles;
  }
  ...
  function handleSubmit(e) {
    ...
    if (Object.keys(nouvelles).length > 0) { setErreurs(nouvelles); return; }
        ...
        <form className="ajouter-film__form" onSubmit={handleSubmit} noValidate>
          ...
          <div className="form__group">
            ...
            <input id="titre" name="titre" type="text"
              ...
              placeholder="Titre du film" value={form.titre} onChange={handleChange} />
          ...
          <div className="form__group">
            ...
            <textarea id="description" name="description"
              ...
              placeholder="Décrivez le film..." value={form.description}
          ...
          <div className="form__group">
            ...
            <input id="posterURL" name="posterURL" type="url"
              className={`form__input ${erreurs.posterURL ? "form__input--erreur" : ""}`}
              placeholder="https://..." value={form.posterURL} onChange={handleChange} />
            {erreurs.posterURL && <span className="form__erreur">{erreurs.posterURL}</span>}
          </div>

          <div className="form__group">
            <label htmlFor="trailerURL" className="form__label">URL de la bande-annonce (YouTube embed) *</label>
            <input id="trailerURL" name="trailerURL" type="url"
              className={`form__input ${erreurs.trailerURL ? "form__input--erreur" : ""}`}
              placeholder="https://www.youtube.com/embed/..." value={form.trailerURL}
              onChange={handleChange} />
            {erreurs.trailerURL && <span className="form__erreur">{erreurs.trailerURL}</span>}
          </div>
          ...
          <div className="form__group">
            ...
            <input id="note" name="note" type="range" className="filtre__range"
              min="0" max="5" step="0.5" value={form.note} onChange={handleChange} />
            <div className="filtre__range-labels"><span>0</span><span>5 ★</span></div>
          </div>

          <button type="submit" className="btn btn--soumettre">Ajouter le film</button>
        </form>
      )}
    </div>
  );
